...
type NemoGuardrailsReconciler struct {
	...
	Namespace string
	ConfigMap string // Name of the operator config map
...
func (r *NemoGuardrailsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	_, _, err = utils.ReconcileConfigMap(ctx, r.Client, nemoGuardrails, caBundleConfigMapName, Version, caBundleTemplate, templateParser.ParseResource)
	if err != nil {
	...
	if utils.RequiresAuth(nemoGuardrails) {
		_, _, err := utils.ReconcileConfigMap(ctx, r.Client, nemoGuardrails, GetRBACConfigName(*nemoGuardrails), Version, "kube-rbac-proxy-config.tmpl.yaml", templateParser.ParseResource)
		if err != nil {
			return ctrl.Result{}, err
		}
