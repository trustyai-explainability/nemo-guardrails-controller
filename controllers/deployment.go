...
import (
	...
	templateParser "github.com/trustyai-explainability/nemo-guardrails-controller/controllers/templates"
...
func (r *NemoGuardrailsReconciler) setAuthConfig(ctx context.Context, nemoGuardrails *nemoguardrailsv1alpha1.NemoGuardrails, deploymentConfig *DeploymentConfig) error {
	...
	authImage, err := utils.GetImageFromConfigMap(ctx, r.Client, configMapKubeRBACProxyImageKey, r.ConfigMap, r.Namespace)
	if err != nil {
		utils.LogErrorRetrieving(ctx, err, "oauth image from configmap", r.ConfigMap, r.Namespace)
		return err
	}
	log.FromContext(ctx).Info("using AuthProxyImage " + authImage + " " + "from config map " + r.Namespace + ":" + r.ConfigMap)
...
func (r *NemoGuardrailsReconciler) createDeployment(ctx context.Context, nemoGuardrails *nemoguardrailsv1alpha1.NemoGuardrails, caBundleInitContainerConfig utils.CABundleInitContainerConfig, configMapsToMount []corev1.ConfigMap) (*appsv1.Deployment, error) {
	...
	nemoGuardrailsImage, err := utils.GetImageFromConfigMap(ctx, r.Client, nemoGuardrailsImageKey, r.ConfigMap, r.Namespace)
	if err != nil {
		utils.LogErrorRetrieving(ctx, err, "nemo-guardrails image from configmap", r.ConfigMap, r.Namespace)
		return nil, err
	}
	if nemoGuardrailsImage == "" {
		err = fmt.Errorf("configmap %s in namespace %s has empty value for key %s", r.ConfigMap, r.Namespace, nemoGuardrailsImageKey)
		utils.LogErrorRetrieving(ctx, err, "nemo-guardrails image from configmap", r.ConfigMap, r.Namespace)
		return nil, err
	}
	containerImages.NemoGuardrailsImage = nemoGuardrailsImage
	log.FromContext(ctx).Info("using NemoGuardrailsImage " + nemoGuardrailsImage + " " + "from config map " + r.Namespace + ":" + r.ConfigMap)
